...
public class PersonDAO {
    ...
    public void update(int id, PersonForm personForm) {
        log.info("Обновление информации пользователя где id=" + id);
        jdbcTemplate.update("UPDATE customers SET firstname=?, lastname=?, age=?, start=?, procent=?, des_score=? WHERE id=?",personForm.getFirstname(), personForm.getLastname(), personForm.getAge(), personForm.getStart(), personForm.getProcent(), personForm.getDes_score(), id);
    }
